...
& $taskNode (Join-Path $taskRoot 'apps\web\tools\copy-cesium.cjs')
if ($LASTEXITCODE -ne 0) { throw 'Could not prepare Cesium assets. Run npm install in apps/web.' }
...
if (!(Get-NetTCPConnection -State Listen -LocalPort $ApiPort -ErrorAction SilentlyContinue)) {
    $taskProcesses.api = (Start-Process -FilePath $taskPython -ArgumentList @('-m','uvicorn','main:app','--host','127.0.0.1','--port',"$ApiPort") -WorkingDirectory (Join-Path $taskRoot 'apps\api') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $taskRoot '.runtime\backend.log') -RedirectStandardError (Join-Path $taskRoot '.runtime\backend-error.log')).Id
}
if (!(Get-NetTCPConnection -State Listen -LocalPort $WebPort -ErrorAction SilentlyContinue)) {
    $taskProcesses.web = (Start-Process -FilePath $taskNode -ArgumentList @('node_modules/next/dist/bin/next','dev','-H','127.0.0.1','-p',"$WebPort") -WorkingDirectory (Join-Path $taskRoot 'apps\web') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $taskRoot '.runtime\frontend.log') -RedirectStandardError (Join-Path $taskRoot '.runtime\frontend-error.log')).Id
}
