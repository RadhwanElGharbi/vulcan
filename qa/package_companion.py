...
files = [ROOT/'Start-Vulcan.ps1', ROOT/'Start VULCAN.cmd', ROOT/'qa/verify_runtime_archives.py']
files += [p for p in (ROOT/'gui-v2/backend').rglob('*') if p.is_file() and p.suffix in {'.py','.ps1','.txt','.json'} and '__pycache__' not in p.parts]
