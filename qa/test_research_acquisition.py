...
def os_grid_fixture():
    folder=ROOT/'qa/fixtures/os_terrain'
...
os._exit(73)
''',encoding='utf-8')
    process=subprocess.run([sys.executable,str(script),str(ROOT/'gui-v2/backend'),str(store.root),str(project),next_job['id'],boundary],capture_output=True,timeout=30)
...
def test_parent_death_terminates_http_child(tmp_path):
    ...
    child=subprocess.Popen([sys.executable,'-c',code,str(ROOT/'gui-v2/backend'),str(parent.pid)],stdout=subprocess.PIPE,text=True)
    try:
...
def test_catalogue_aliases_do_not_substitute_products():
    ...
    spec=importlib.util.spec_from_file_location('catalogue_audit',ROOT/'qa/audit_catalogue.py')
