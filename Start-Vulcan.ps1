...
if (!(Test-Path -LiteralPath $taskReadyMarker)) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Runtime installation failed. Run Start VULCAN.cmd to retry.' }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Python package installation failed. Run Start VULCAN.cmd to retry.' }
...
$taskProcess = Start-Process -FilePath $taskPython -ArgumentList @('-m','uvicorn','main:app','--host','127.0.0.1','--port',"$ApiPort") -WorkingDirectory (Join-Path $taskRoot 'gui-v2\backend') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $taskState 'companion.log') -RedirectStandardError (Join-Path $taskState 'companion-error.log')
$taskReady = $false
