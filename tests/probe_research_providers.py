...
if __name__ == "__main__":
    Store()
    previous_path=ROOT/"tests/research-provider-discovery.json"
    result=[]
    import argparse
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        ...
        for future in concurrent.futures.as_completed(jobs):
            ...
            atomic_json(ROOT/"tests/research-provider-discovery.json",{"checked_at":now(),"scope":"Discovery only; not end-to-end acquisition qualification", "products":sorted(result,key=lambda r:r["product"])})
