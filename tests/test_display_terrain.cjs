...
const compiled = ts.transpileModule(fs.readFileSync('apps/web/src/lib/map/terrain.ts','utf8'), {
...
(async()=>{
  ...
  failed.dispose();
  const samplerCode=ts.transpileModule(fs.readFileSync('apps/web/src/lib/terrainSampler.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText;
